...
import { llmSuggestDB } from "../models/db.js";
import * as llmSuggestHelper from "../utils/llmSuggestHelper.js";
...
export async function deleteHistory(userId: string, conversationId: string) {
    const limit = 400;
    const query = llmSuggestDB.where('userId', '==', userId).where('conversationId', '==', conversationId).limit(limit);
...
export async function queryHistory(userId: string, conversationId: string): Promise<LLMHistory[]> {
    ...
    const query = llmSuggestDB.where('userId', '==', userId).where('conversationId', '==', conversationId);
    try {
        const snapshot = await query.get();
...
async function addHistory(llmHistory: LLMHistory): Promise<[boolean, string]> {
    try {
        const docRef = await llmSuggestDB.add(llmHistory);
        if(docRef.id) {
            docRef.update({
                llmChatId: docRef.id
            });
...
export async function helpfulTelemetry(llmChatId: string, isHelpful: boolean): Promise<boolean> {
    try {
        const doc = await llmSuggestDB.doc(llmChatId).get();
        if(doc.exists) {
            await llmSuggestDB.doc(llmChatId).update({
                isHelpful: isHelpful
            });
...
export async function emotionAnalysis(userId: string, conversationId: string, userContextInput?: string): Promise<[boolean, string, string]> {
    let messageList = await llmSuggestHelper.fetchConversation(userId, conversationId);
    ...
    messageList = llmSuggestHelper.extractLatestMessagesBlock(6, messageList);
    let flatMessageList = llmSuggestHelper.flattenSimplifiedMessages(messageList);
...
async function emotionAnalysisImpl(conversation: string[], userContext: string, enableDebug?: boolean): Promise<[boolean, string]> {
    ...
    const lowendPermit = ["gemma-3-4b-it"];
    ...
    for(const m of highendPermit) {
        try {
            ...
            if(response.text) {
                emotionRecognitionResponse = llmSuggestHelper.trimMarkdownJson(response.text);
            }
            errorString = "";
    ...
    for(const m of highendPermit) {
        try {
            ...
            if(response.text) {
                ecrResponse = llmSuggestHelper.trimMarkdownJson(response.text);
            }
            errorString = "";
    ...
    for(const m of lowendPermit) {
        try {
            ...
            if(response.text) {
                generation = llmSuggestHelper.trimMarkdownJson(response.text);
            }
            errorString = "";
