...
function MatchingPage() {
		...
		const fetchMatches = async () => {
			try {
				...
				if (matchesArray && matchesArray.length > 0) {
					...
					setHasMoreMatches(transformedMatches.length > 0);
				} else {
	...
	// Show "No More Matches" when hasMoreMatches is false or no current match
						...
						<div className='mb-6'>
							...
							<div className='p-4 bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl text-white'>
								<p className='text-lg font-semibold capitalize'>
									{typeof currentMatch.workVibe === "object"
										? (currentMatch.workVibe as WorkVibe)
												.type
