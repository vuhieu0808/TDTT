...
import { llmChatServices } from "@/services/llmChatServices";
...
function llmChatModal({
	...
	useEffect(() => {
		const fetchHistory = async () => {
			...
			try {
				const historyString = await llmChatServices.getHistory(
					conversationId
				);
				console.log("Fetched LLM chat history:", historyString);
				...
				try {
					...
					if (!historyArray || historyArray.length === 0) {
						console.log(
							"History array is empty, showing welcome message"
						);
						showWelcomeMessage();
						return;
					}
					...
					const parsedMessages: Message[] = historyArray.map(
						(item) => {
					...
					// Set messages with welcome message at the beginning
					setMessages([
						{
							role: "assistant",
							content:
					...
					console.log(
						`Loaded ${parsedMessages.length} messages from history`
					);
				} catch (parseError) {
				...
				// Show welcome message instead of error toast for better UX
				showWelcomeMessage();
			} finally {
